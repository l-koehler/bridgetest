...
use azalea::protocol::packets::game::c_level_chunk_with_light::{
    ClientboundLevelChunkPacketData, ClientboundLevelChunkWithLight,
};
...
pub async fn death(
    ...
    mt_server_state.mt_clientside_pos = (
        respawn_pos.0 * 10.0,
        respawn_pos.1 * 10.0,
        respawn_pos.2 * 10.0,
    );
...
pub async fn update_inventory(
    ...
    let unchanged_fields: Vec<&str> = settings::ALL_INV_FIELDS
        .into_iter()
...
pub async fn add_entity(
    optional_packet: Option<&ClientboundAddEntity>,
    conn: &mut LuantiConnection,
    mt_server_state: &mut MTServerState,
) {
    let is_player: bool;
    ...
    match mc_client.menu() {
        inventory::Menu::Player(serverside_inventory) => {
            ...
            if serverside_inventory.armor.as_slice()
                != mt_server_state.mt_clientside_player_inv.armor.as_slice()
            {

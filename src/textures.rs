...
use luanti_protocol::types::{aabb3f, MediaAnnouncement, MediaFileData, NodeBox, NodeBoxFixed, TileAnimationParams, TileDef, AlignStyle, DrawType};
...
impl LuantiTexture {
    ...
    pub fn from_absolute(apath: PathBuf, model_mode: bool) -> LuantiTexture {
        ...
        let rel_path = apath.strip_prefix(relative_root).unwrap().to_str().unwrap().to_owned(); 
...
pub fn get_announcement() -> ToClientCommand {
    ...
    for texture in get_texture_iterator_recursive(texture_root, settings::TEXTURE_MAX_RECURSION, false) {
...
pub fn get_texture_iterator_recursive(path: PathBuf, limit: u8, model_mode: bool) -> Vec<LuantiTexture> {
    ...
    for entry in path.read_dir().unwrap() {
        ...
        if entry_u.file_type().unwrap().is_dir() {
            ret.extend(get_texture_iterator_recursive(entry_u.path(), limit-1, model_mode));
        };
...
impl BlockMapping {
    ...
    pub fn to_safe_cube(&self) -> String {
        return format!(
            ...
            self.textures.get(&Direction::North).unwrap().to_luanti_safe(),
            self.textures.get(&Direction::East).unwrap().to_luanti_safe()
        )
    }
}
...
pub fn load_block_mappings(nodebox_mapping: &HashMap<String, NodeBox>) -> HashMap<String, BlockMapping> {
...
fn generate_nodebox(cuboids: Vec<[i8; 6]>) -> NodeBox {
    ...
    return NodeBox::Fixed(NodeBoxFixed {
        fixed: ab_bounds
    })
}
...
pub fn get_empty_tiledefs() -> [TileDef; 6] {
    ...
    return [td.clone(), td.clone(), td.clone(), td.clone(), td.clone(), td]
}
...
pub async fn fetch_models() {
    ...
    // attempt to get zip
    let model_url = "https://codeberg.org/mineclonia/mineclonia/archive/main:mods/ENTITIES/mobs_mc/models.zip";
    let resp = reqwest::get(model_url).await.expect("Failed to request texture pack!");
